...
std::hash_map<UINT32, ShadowPage>::value_type ShadowMem::GetPageTableEntry(UINT32 virtual_address){
	...
	if(it == page_table_.end()){
		ShadowPage shadow_page = (ShadowPage)malloc(PAGE_SIZE * sizeof(InstructionTrace *));
		if(shadow_page == NULL){
			cerr<< "allocate page table failed" << endl;
		}
		memset(shadow_page, 0, PAGE_SIZE * sizeof(InstructionTrace *));
...
ShadowPage ShadowMem::CheckTaint(UINT32 addr, UINT32 length)
{
	auto page_entry = GetPageTableEntry(addr);
	ShadowPage shadow_page = page_entry.second;
	for(UINT32 i=addr; i<(addr+length); i++){
		if(shadow_page[VIRT2OFFSET(i)] != NULL){
			return shadow_page[VIRT2OFFSET(i)];
		}
	}
	return 0;
}
...
VOID ShadowMem::RemoveTaint(UINT32 addr,UINT32 length)
{
	...
	ShadowPage shadow_page = page_entry.second;
	for(UINT32 i=addr; i<(addr+length); i++){
		shadow_page[VIRT2OFFSET(i)] = NULl;
	}
	D(std::cout << std::hex << "\t\t\t--" << addr<<"["<<length<< "] is now freed" << std::endl;)
}

VOID ShadowMem::AddTaint(UINT32 addr, UINT32 length, InstructionTrace * trace)
{
	auto page_entry = GetPageTableEntry(addr);
	ShadowPage shadow_page = page_entry.second;
	for(UINT32 i=addr; i<(addr+length); i++){
		shadow_page[VIRT2OFFSET(i)] = trace;
	}
